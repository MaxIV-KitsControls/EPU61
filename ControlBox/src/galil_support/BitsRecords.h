...
BEGIN_BITS_RECORD(BR_GenIO)
  ...
  //- 10 bytes of general output
	...
	//Reserved 16 bytes (8 words)
	IGNORE_MEMBER(res_0, 16, unsigned short);
	IGNORE_MEMBER(res_1, 16, unsigned short);
	IGNORE_MEMBER(res_2, 16, unsigned short);
	IGNORE_MEMBER(res_3, 16, unsigned short);
	IGNORE_MEMBER(res_4, 16, unsigned short);
	IGNORE_MEMBER(res_5, 16, unsigned short);
	IGNORE_MEMBER(res_6, 16, unsigned short);
	IGNORE_MEMBER(res_7, 16, unsigned short);

	//Ethernet status 8 bytes
	IGNORE_MEMBER(ethernet_A, 8, unsigned char); //- ethernet handle A status
	IGNORE_MEMBER(ethernet_B, 8, unsigned char); //- ethernet handle B status
	IGNORE_MEMBER(ethernet_C, 8, unsigned char); //- ethernet handle C status
	IGNORE_MEMBER(ethernet_D, 8, unsigned char); //- ethernet handle D status
	IGNORE_MEMBER(ethernet_E, 8, unsigned char); //- ethernet handle E status
	IGNORE_MEMBER(ethernet_F, 8, unsigned char); //- ethernet handle F status
	IGNORE_MEMBER(ethernet_G, 8, unsigned char); //- ethernet handle G status
	IGNORE_MEMBER(ethernet_H, 8, unsigned char); //- ethernet handle H status

//- byte : error code
	MEMBER(err_code, 8, unsigned char);  //- error code see TC command
	 //- byte : general status information
	IGNORE_MEMBER(running_7, 1, bool);	 //- thread 7 running
	IGNORE_MEMBER(running_6, 1, bool);	 //- thread 6 running
	IGNORE_MEMBER(running_5, 1, bool);	 //- thread 5 running
	IGNORE_MEMBER(running_4, 1, bool);	 //- thread 4 running
	IGNORE_MEMBER(running_3, 1, bool);	 //- thread 3 running
	IGNORE_MEMBER(running_2, 1, bool);	 //- thread 2 running
	IGNORE_MEMBER(running_1, 1, bool);	 //- thread 1 running
	MEMBER(program_running, 1, bool);	 //- is ucode running (thread 0 running)
	//Unsigned Long amplifier status
	MEMBER(amp_status, 32, yat::int32);
	//Unsigned Long segment count for Contour Mode
	//MEMBER(segment_count, 32, yat::int32);
	//word bufferspace remaining for Contour Mode
	//MEMBER(buff_space, 16, unsigned short);
...
BEGIN_BITS_RECORD_EXTRACTOR(BR_GenIO)
  ...
  EXTRACT_MEMBER(amp_status);
  //EXTRACT_MEMBER(segment_count);
  //EXTRACT_MEMBER(buff_space);
    //SKIP_BITS(80);
...
BEGIN_BITS_RECORD_DUMP(BR_GenIO)
  ...
  DUMP_MEMBER(amp_status);
  //DUMP_MEMBER(segment_count);
  //DUMP_MEMBER(buff_space);
    //DUMP_SKIP_BITS(80);
...
//******************************************************************************
//- BR_SPlane - BR_SPlane - BR_SPlane - BR_SPlane - BR_SPlane - BR_SPlane - BR_S
//******************************************************************************
//------------------------------------------------------------------------------
//- BITS RECORD: S Plane
//------------------------------------------------------------------------------
BEGIN_BITS_RECORD(BR_SPlane)
	//-- byte 0 ------------------------
  IGNORE_MEMBER(s_plane_0, 16, unsigned short); //- s plane 0
  IGNORE_MEMBER(s_plane_1, 16, unsigned short); //- s plane 1
  IGNORE_MEMBER(s_plane_2, 32, yat::int32);     //- s plane 2
  IGNORE_MEMBER(s_plane_3, 16, unsigned short); //- s plane 3
END_BITS_RECORD(BR_SPlane)

//------------------------------------------------------------------------------
//- EXTRACTOR: BR_SPlane
//------------------------------------------------------------------------------
BEGIN_BITS_RECORD_EXTRACTOR(BR_SPlane)
  SKIP_BITS(80);
END_BITS_RECORD_EXTRACTOR(BR_SPlane)

//------------------------------------------------------------------------------
//- DUMP: BR_SPlane
//------------------------------------------------------------------------------
BEGIN_BITS_RECORD_DUMP(BR_SPlane)
  DUMP_SKIP_BITS(80);
END_BITS_RECORD_DUMP(BR_SPlane)

//******************************************************************************
//- CLASS: SPlane (humanly usable mapping for BR_SPlane)
//******************************************************************************
class SPlane: public BR_SPlane
{
public:
  //--------------------------------------------
  SPlane ()
...
//******************************************************************************
//- BR_TPlane - BR_TPlane - BR_TPlane - BR_TPlane - BR_TPlane - BR_TPlane - BR_T
//******************************************************************************
//------------------------------------------------------------------------------
//- BITS RECORD: T Plane
//------------------------------------------------------------------------------
BEGIN_BITS_RECORD(BR_TPlane)
	//-- byte 0 ------------------------
  IGNORE_MEMBER(t_plane_0, 16, unsigned short); //- t plane 0
  IGNORE_MEMBER(t_plane_1, 16, unsigned short); //- t plane 1
  IGNORE_MEMBER(t_plane_2, 32, yat::int32);     //- t plane 2
  IGNORE_MEMBER(t_plane_3, 16, unsigned short); //- t plane 3
END_BITS_RECORD(BR_TPlane)

//------------------------------------------------------------------------------
//- EXTRACTOR: BR_TPlane
//------------------------------------------------------------------------------
BEGIN_BITS_RECORD_EXTRACTOR(BR_TPlane)
  SKIP_BITS(80);
END_BITS_RECORD_EXTRACTOR(BR_TPlane)

//------------------------------------------------------------------------------
//- DUMP: BR_TPlane
//------------------------------------------------------------------------------
BEGIN_BITS_RECORD_DUMP(BR_TPlane)
  DUMP_SKIP_BITS(80);
END_BITS_RECORD_DUMP(BR_TPlane)

//******************************************************************************
//- CLASS: TPlane (humanly usable mapping for BR_TPlane)
//******************************************************************************
class TPlane: public BR_TPlane
{
public:
  //--------------------------------------------
  TPlane ()
...
//******************************************************************************
//- BR_FirmwareAxisState - BR_FirmwareAxisState - BR_FirmwareAxisState - BR_Firm
//******************************************************************************
//------------------------------------------------------------------------------
...
BEGIN_BITS_RECORD(BR_FirmwareAxisState)
  ...
  MEMBER(HM_phase_3, 1, bool); 		  			//- HoMe in progress phase 3
  ...
  MEMBER(reserved_1, 1, bool); 		//- N/A
  MEMBER(reserved_2, 1, bool); 		//- N/A
  ...
  MEMBER(ref_pos, 32, yat::int32); 	          //- axis reference position
  //- axis motor position signed long
  MEMBER(mot_pos, 32, yat::int32);         //- axis motor position see TP (encoder position)
  //- axis position error signed long
  MEMBER(pos_err, 32, yat::int32); 	          //- axis position error see TE
  //- axis auxiliary position signed long
  MEMBER(aux_pos, 32, yat::int32); 	          //- axis auxiliary encoder position see TD (motor position for steppers)
  //- axis velocity signed long
  MEMBER(velocity, 32, yat::int32);	          //- axis velocity see TV instant velocity if encoder present, 0 otherwise
  //- axis torque signed word
  MEMBER(torque, 32, yat::int32);			        //- axis torque
  //- axis analog input value signed word
  MEMBER(ana_in, 16, short);			        //- axis input value
  //- axis Hall input status byte
  MEMBER(hall_status, 8, unsigned char); 	  //- axis hall input status
  //- reserved byte
  MEMBER(reserved_3, 8, unsigned char); 		//- N/A

  MEMBER(user_def_za, 32, yat::int32); 		//- User defined variable (ZA)
...
BEGIN_BITS_RECORD_EXTRACTOR(BR_FirmwareAxisState)
  ...
  EXTRACT_MEMBER(reserved_1); 		//- N/A
  EXTRACT_MEMBER(reserved_2); 		//- N/A
  //SKIP_BITS(2);
  ...
  EXTRACT_MEMBER(hall_status);
  //SKIP_BITS(8);
EXTRACT_MEMBER(reserved_3);
  EXTRACT_MEMBER(user_def_za);
...
BEGIN_BITS_RECORD_DUMP(BR_FirmwareAxisState)
  ...
  DUMP_MEMBER(reserved_1);
  DUMP_MEMBER(reserved_2);
  //DUMP_SKIP_BITS(2);
  ...
  DUMP_MEMBER(torque);
  DUMP_MEMBER(ana_in);
  DUMP_MEMBER(hall_status);
  DUMP_MEMBER(reserved_3);
//  DUMP_SKIP_BITS(8);
  DUMP_MEMBER(user_def_za);
...
BEGIN_BITS_RECORD_EXTRACTOR(BR_QRBlock)
  ...
  EXTRACT_MEMBER(gio);
#if defined(_USE_ASYNCH_DR_)
  EXTRACT_MEMBER(splane);
  EXTRACT_MEMBER(tplane);
#endif
  EXTRACT_MEMBER(ab_a);
  EXTRACT_MEMBER(ab_b);
  EXTRACT_MEMBER(ab_c);
...
BEGIN_BITS_RECORD(BR_UCodeAxisState)
  ...
  MEMBER(pos_locked, 1, bool); 		     //- lock pos. status (enabled or disabled?)
  MEMBER(is_initialised, 1, bool); 		 //- valeurs importantes initialisees par le device
  //- byte 3
...
BEGIN_BITS_RECORD(BR_UCodeAxisCommand)
	 //-- byte 0
	 MEMBER(cmd_updt_gearing_mode, 1, bool); //- update for gearing
	 MEMBER(cmd_motor_off, 1, bool);			 //- motor off
	 MEMBER(cmd_motor_on, 1, bool); 			 //- rmotor on
	 MEMBER(cmd_init_ref_pos, 1, bool); 	 //- initialise reference position
	 MEMBER(cmd_pos, 1, bool);						 //- absolute positionning
	 MEMBER(cmd_jog_minus, 1, bool);			 //- jog negative direction
	 MEMBER(cmd_jog_plus, 1, bool); 			 //- jog positive direction
	 MEMBER(cmd_stop, 1, bool);						 //- stop
	 //- byte 1 
	 ...
	 MEMBER(cmd_lock_pos, 1, bool);				 //- enable/disable lock pos.
	 //- byte 2
	 IGNORE_MEMBER(reserved_23, 1, bool);  //- N/A
	 IGNORE_MEMBER(reserved_22, 1, bool);  //- N/A
	 IGNORE_MEMBER(reserved_21, 1, bool);  //- N/A
...
BEGIN_BITS_RECORD_EXTRACTOR(BR_UCodeAxisCommand)	
  EXTRACT_MEMBER(cmd_updt_gearing_mode);
  EXTRACT_MEMBER(cmd_motor_off);
  EXTRACT_MEMBER(cmd_motor_on); 			
  EXTRACT_MEMBER(cmd_init_ref_pos);
  EXTRACT_MEMBER(cmd_pos);
  EXTRACT_MEMBER(cmd_jog_minus);
  EXTRACT_MEMBER(cmd_jog_plus); 
  EXTRACT_MEMBER(cmd_stop);
  SKIP_BITS(7); 
  EXTRACT_MEMBER(cmd_lock_pos);
  SKIP_BITS(16); 
...
BEGIN_BITS_RECORD_DUMP(BR_UCodeAxisCommand)
  DUMP_MEMBER(cmd_updt_gearing_mode);
  DUMP_MEMBER(cmd_motor_off);
  DUMP_MEMBER(cmd_motor_on); 			
  DUMP_MEMBER(cmd_init_ref_pos);
  DUMP_MEMBER(cmd_pos);
  DUMP_MEMBER(cmd_jog_minus);
  DUMP_MEMBER(cmd_jog_plus); 
  DUMP_MEMBER(cmd_stop);
  DUMP_MEMBER(cmd_lock_pos);
...
public:
  ...
  HardwareStatus ()
  //--------------------------------------------
  : tc_error(0), tc_error_str(galil::tc_error_txt[0])
  { 
    //-noop ctor 
  }
  //--------------------------------------------
