...
void GalilCommunicator::start_gap_movement(const std::vector<double> &offsets,
        double center, double gap, double taper, double gapSpeed,
        double taperSpeed, double gapAcc, double taperAcc)
{
...
void GalilCommunicator::start_phase_movement(const std::vector<double> &offsets,
        short phaseMode, const std::vector<double> &finalAxisPos, double phaseSpeed,
        double phaseAcc)
{
    ...
    if(offsets.size() != 4 || finalAxisPos.size() != 4)
    ...
    for(int i = 0; i < 4; i++)
    {
        cmd << "PhDest[" << i << "]=" << fixed << finalAxisPos[i] << ";";
    }
