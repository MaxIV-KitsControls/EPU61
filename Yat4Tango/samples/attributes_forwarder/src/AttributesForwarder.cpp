...
void AttributesForwarder::init_device()
{
  ...
  yat4tango::TraceHelper t("AttributesForwarder::init_device", this);
  ...
  for ( size_t i = 0; i < forwardedAttributes.size(); i++)
  {
    ...
    INFO_STREAM << "forwarding attribute "
                << fai[i].url
                << (fai[i].lan == yat4tango::KeepOriginalAttributeName ? " keeping orignal its name" : " as ")
  ...
  {
    this->set_state(Tango::FAULT);
    this->set_status("failed to foarward the specified attributes - unknown exception caught");
    return;
  }
  ...
  try
  {
    for (size_t i = 0; i < 3; ++i)
    {
      ...
      //- specify the dyn. attr. data type
      dai.tai.data_type = Tango::DEV_SHORT;
      //- specify the dyn. attr. data format
